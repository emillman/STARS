...
function [ success ] = storeFile( context, file )
    ...
    if ~context.remote_store
        warning('STARS:storeFile','cannot store a file when not using a remote store, ignoring call' );
    else
        ...
        if ~isempty( split_idx )
            ...
            cmd_line = sprintf( 'ssh %s "mkdir -p %s/%s >/dev/null" && scp %s/%s%s %s:%s/%s > /dev/null', ...
                host_name, remote_path, path, ...
                context.temp_path, file_name, ext, ...
                host_name, remote_path, file );
            
            if system( cmd_line ) == 0
                success = true;
            end
        else
            warning('STARS:storeFile','store_path does not appear to be a remote location, ignoring call');
        end
    end
end
